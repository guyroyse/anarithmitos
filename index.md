---
---

# Anarithmitos Sector

![alt text]({{ "/anarithmitos-sector.png" | absolute_url }}){:height="100%" width="100%"}

## Systems

- [Cor Caroli]({{ "/systems/cor-caroli" | absolute_url }})
- [Cygnus 76]({{ "/systems/cygnus-76" | absolute_url }})
- [Draper Omega]({{ "/systems/draper-omega" | absolute_url }})
- [Lotus Pegasi]({{ "/systems/lotus-pegasi" | absolute_url }})
- [Zealand Prime]({{ "/systems/zealand-prime" | absolute_url }})

## Factions

- [Caroli Holdings]({{ "/factions/caroli-holdings" | absolute_url }})
- [Church of Cygnus Dei]({{ "/factions/church-of-cygnus-dei" | absolute_url }})
- [Linear Concord]({{ "/factions/linear-concord" | absolute_url }})
